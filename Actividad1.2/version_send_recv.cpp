    ...
    do {
        ...
        if (opcion >= 1 && opcion <= 5) {
            ...
            if (opcion == 1) {
                ...
                if (rank != 0) {
                    MPI_Send(A_local, n_local, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD);
                    MPI_Send(B_local, n_local, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD);
                } else {
                    for (int i = 0; i < n_local; i++) {
                        A_global[i] = A_local[i];
                        B_global[i] = B_local[i];
                    }
                    for (int src = 1; src < size; src++) {
                        MPI_Recv(A_global + src * n_local, n_local, MPI_DOUBLE, src, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        MPI_Recv(B_global + src * n_local, n_local, MPI_DOUBLE, src, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    }
                }
            } else {
                if (rank == 0) {
                    ...
                    for (int dest = 1; dest < size; dest++) {
                        MPI_Send(A_global + dest * n_local, n_local, MPI_DOUBLE, dest, 10, MPI_COMM_WORLD);
                        MPI_Send(B_global + dest * n_local, n_local, MPI_DOUBLE, dest, 20, MPI_COMM_WORLD);
                    }
                } else {
                    MPI_Recv(A_local, n_local, MPI_DOUBLE, 0, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    MPI_Recv(B_local, n_local, MPI_DOUBLE, 0, 20, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                }
                ...
                if (rank != 0) {
                    MPI_Send(C_local, n_local, MPI_DOUBLE, 0, 30, MPI_COMM_WORLD);
                } else {
                    for (int i = 0; i < n_local; i++) C_global[i] = C_local[i];
                    for (int src = 1; src < size; src++) {
                        MPI_Recv(C_global + src * n_local, n_local, MPI_DOUBLE, src, 30, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    }
                }
            }
